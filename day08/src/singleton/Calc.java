...
public class Calc {
	// ---------singleton
	// 공통 기능을 포함할 클래스 안에 스스로를 객체 static 선언
	/*
	 ...
	 *  코드 상의 논리적인 연관관계를 위해 하나의 파일에서 정의하는 것일 뿐,
	 *  실제로는 Calc 클래스 자체에서 독립된 객체가 되는 것이다.
	 */
	 /* 
	 *  - Calc 클래스 이외의 여러 곳에서 new 연산자를 사용하여
	 *  객체 할당이 가능하면 Singleton이라는 개념 자체가 성립되지않으므로,
	...
	// 객체에 null을 대입해서 메모리에서 삭제한다. 정석이긴 하나 잘 안쓰긴 함.
	...
	public int divde( int x, int y ) { return x / y; }
}
